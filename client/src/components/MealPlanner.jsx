...
export default function MealPlanner() {
  ...
  const setPlanMutation = useMutation({
    mutationFn: ({ day, recipeId, recipeTitle, imageUrl }) =>
      api
        .put(`/mealplan/${day}`, { recipeId, recipeTitle, imageUrl })
        .then((r) => r.data),
    onSuccess: () => {
      ...
      <header className="glass sticky top-14 z-40 border-b border-white/20">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <h1 className="text-lg font-bold text-gray-900 flex-1 flex items-center gap-2">
            <Calendar className="w-5 h-5 text-primary" /> Weekly Meal Planner
          </h1>
        </div>
      </header>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3 p-4">
                  <div
                    className={`flex-shrink-0 text-center w-12 ${isToday ? "text-primary" : "text-gray-500"}`}
                  >
                    <div className="text-xs font-semibold uppercase tracking-wide">
                      {DAY_LABELS[day]}
                    </div>
                    <div
                      className={`text-sm font-bold ${isToday ? "text-primary" : "text-gray-400"}`}
                    >
                      {dateStr}
                    </div>
                    {isToday && (
                  ...
                  <div className="flex-1 min-w-0">
                    {planned?.recipeId ? (
                      ...
                      >
                        ...
                        <span className="font-semibold text-gray-900 text-sm line-clamp-2 flex-1">
                          {planned.recipeTitle}
                        </span>
                      </button>
                    ) : (

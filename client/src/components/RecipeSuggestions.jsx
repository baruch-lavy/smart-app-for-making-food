...
export default function RecipeSuggestions() {
  ...
  React.useEffect(() => {
    if (profileData?.favorites)
      setFavorites(new Set(profileData.favorites.map((f) => f._id || f)));
  }, [profileData]);
  ...
  const searchMutation = useMutation({
    mutationFn: (q) =>
      api.get(`/recipes/search?q=${encodeURIComponent(q)}`).then((r) => r.data),
    onSuccess: (data) => {
  ...
  const importMutation = useMutation({
    mutationFn: ({ id, isSpoonacular }) => {
      const endpoint = isSpoonacular
        ? `/recipes/spoonacular/${id}`
        : `/recipes/mealdb/${id}`;
  ...
  const addToShoppingMutation = useMutation({
    mutationFn: (recipeId) =>
      api.post(`/shopping/from-recipe/${recipeId}`).then((r) => r.data),
  ...
  const favoriteMutation = useMutation({
    mutationFn: (recipeId) => {
      if (favorites.has(recipeId))
        return api.delete(`/users/favorites/${recipeId}`).then((r) => r.data);
  ...
  const filteredSuggestions = [...suggestions]
    ...
    .sort((a, b) => {
      if (sortBy === "popular")
        return (
          (b.metrics?.popularityScore || 0) - (a.metrics?.popularityScore || 0)
        );
      if (sortBy === "fastest")
  ...
  const displayList =
    activeTab === "search" ? searchResults : filteredSuggestions;
              ...
              {selectedIngredients.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-3">
                  {selectedIngredients.map((ing) => (
                    <span
                      key={ing}
                      className="flex items-center gap-1 px-3 py-1 bg-orange-100 text-primary rounded-full text-sm"
                    >
                      {ing}
          ...
          !suggestMutation.isPending && (
            <div className="text-center py-12 text-gray-400">
              ...
              <p className="font-medium text-gray-600">
                Select ingredients and get smart recipe ideas
              </p>
              <p className="text-sm mt-1">
                Or search the web above for any recipe
              </p>
            </div>
          )}

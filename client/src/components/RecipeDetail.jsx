...
export default function RecipeDetail() {
  ...
  const smartAddMutation = useMutation({
    mutationFn: () =>
      api.post(`/shopping/from-recipe/${id}`).then((r) => r.data),
    onSuccess: (data) => {
  ...
  const manualAddMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries(["shopping"]);
      addToast("Added to shopping list!");
    },
  });

  const toggleCheck = (idx) =>
    setChecked((c) =>
      c.includes(idx) ? c.filter((i) => i !== idx) : [...c, idx],
    );
  ...
  if (!recipe)
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-gray-400">Recipe not found</div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gradient-mesh pb-8">
      ...
      <div className="max-w-2xl mx-auto px-4 pt-5">
        <div className="flex items-center gap-2 mb-3">
          <span className="text-sm bg-orange-100 text-primary rounded-full px-3 py-1 font-medium">
            {recipe.cuisine}
          </span>
          <span
            className={`text-sm rounded-full px-3 py-1 font-medium ${DIFF_COLOR[recipe.difficulty]}`}
          >
            {recipe.difficulty}
          </span>
        </div>
        ...
        <div className="bg-orange-50 rounded-2xl p-4 flex flex-wrap gap-5 mb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="font-bold text-sm">{recipe.prepTime} min</div>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="font-bold text-sm">{recipe.cookingTime} min</div>
            </div>
          </div>
          <div className="flex items-center gap-2">
          ...
          <div className="bg-red-50 border border-red-100 rounded-2xl p-4 mb-4">
            ...
            <div className="flex flex-wrap gap-1.5 mb-3">
              {missingData.missing.map((ing, i) => (
                <span
                  key={i}
                  className="text-xs bg-red-100 text-red-700 rounded-full px-2.5 py-1"
                >
                  {ing.name}
                  {ing.amount
            ...
            {addResult ? (
              <p className="text-sm text-green-600 font-medium">
                ✅ {addResult.added} item{addResult.added !== 1 ? "s" : ""}{" "}
                added
                {addResult.skipped > 0 && (
                  <span className="text-gray-400">
                    {" "}
                    ({addResult.skipped} already there)
                  </span>
                )}
              </p>
            ) : (
              ...
              >
                ...
                {smartAddMutation.isPending
                  ? "Adding..."
                  : `Add all ${missingCount} to Shopping List`}
              </button>
            )}
          </div>
        )}
        ...
        <div className="card p-5 mb-4">
          ...
          <div className="space-y-2">
            {recipe.ingredients.map((ing, i) => {
              const isMissing = missingData?.missing?.some(
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={`flex-1 text-sm ${checked.includes(i) ? "line-through text-gray-400" : isMissing ? "text-red-600" : "text-gray-700"}`}
                  >
                    {ing.name}
                    {isMissing && (
        ...
        <div className="flex gap-3">
          <button
            onClick={addUncheckedToShopping}
          ...
          <button
            onClick={() => navigate(`/cook/${recipe._id}`)}

...
export default function RecipeCard({
  ...
  const totalTime = (recipe.prepTime || 0) + (recipe.cookingTime || recipe.readyInMinutes || 0)
  ...
  const isExternalResult = !recipe._id && (recipe.spoonacularId || recipe.mealdbId)
  ...
  return (
    <div
      className="bg-white rounded-2xl border border-gray-200 overflow-hidden hover:shadow-lg transition-all cursor-pointer active:scale-[0.99]"
      onClick={handleClick}
    >
      <div className="relative">
        ...
        {isExternalResult && (
          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/50 to-transparent px-3 py-2">
            <span className="text-white text-xs font-medium">Tap to import recipe →</span>
          </div>
        )}
      </div>

      <div className="p-4">
        <h4 className="font-bold text-gray-900 line-clamp-2 mb-2">{recipe.title}</h4>
        ...
        <div className="flex items-center gap-2 mb-3 flex-wrap">
          ...
          {recipe.difficulty && (
            <span className={`text-xs rounded-full px-2 py-0.5 ${DIFF_COLOR[recipe.difficulty] || 'text-gray-600 bg-gray-100'}`}>
              {recipe.difficulty}
            </span>
          )}
          {source && (
        ...
        <div className="flex items-center justify-between">
          ...
          {showMissing && missing > 0 && (
            <button
              onClick={e => { e.stopPropagation(); onAddToShopping?.(recipe._id) }}

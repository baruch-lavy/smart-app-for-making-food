function buildSvgFallback(prompt) {
  const escaped = String(prompt || "Recipe")
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;");
...
function buildPollinationsUrl(prompt, options = {}) {
  ...
  return `https://image.pollinations.ai/prompt/${encodedPrompt}?model=${encodeURIComponent(model)}&width=${width}&height=${height}&seed=${seed}&nologo=true&referrer=cooking-app`;
}

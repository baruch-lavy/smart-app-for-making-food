...
const PREFERRED_SEARCH_PROVIDER = (process.env.AI_SEARCH_PROVIDER || 'auto').toLowerCase();
const NOT_KID_FRIENDLY_TERMS = ['wine', 'beer', 'vodka', 'rum', 'tequila', 'bourbon', 'whiskey', 'whisky', 'espresso', 'coffee rub', 'chili flakes', 'ghost pepper', 'jalapeno', 'habanero', 'serrano', 'flambe'];
...
function normalizeIngredient(entry) {
  ...
  const name = normalizeText(entry.name || entry.text || entry.ingredient || '');
  if (!name) return null;
...
function normalizeStep(entry, index) {
  ...
  const instruction = normalizeText(entry.text || entry.instruction || entry.name || '');
  if (!instruction) return null;
  return {
...
function extractRecipeFromHtml(html, url) {
  ...
  const steps = instructionsRaw
    .map((entry, index) => {
      if (typeof entry === 'object' && Array.isArray(entry.itemListElement)) {
        return entry.itemListElement.map((stepEntry, nestedIndex) => normalizeStep(stepEntry, nestedIndex));
      }
  ...
  return {
    title: normalizeText(recipeJson?.name || titleMatch?.[1] || 'Internet recipe'),
    description: normalizeText(recipeJson?.description || descMatch?.[1] || 'Recipe adapted from internet sources.'),
    ...
    servings: Number(String(recipeJson?.recipeYield || '2').match(/\d+/)?.[0] || 2),
    ingredients,
    steps,
    tags: [siteNameFromUrl(url)],
    nutritionInfo: {
      calories: Number(String(recipeJson?.nutrition?.calories || '').match(/\d+/)?.[0] || 0),
      protein: Number(String(recipeJson?.nutrition?.proteinContent || '').match(/\d+/)?.[0] || 0),
      carbs: Number(String(recipeJson?.nutrition?.carbohydrateContent || '').match(/\d+/)?.[0] || 0),
      fat: Number(String(recipeJson?.nutrition?.fatContent || '').match(/\d+/)?.[0] || 0),
    },
    source: {
    ...
    sourceImageUrl: imageMatch?.[1] || (Array.isArray(recipeJson?.image) ? recipeJson.image[0] : recipeJson?.image?.url || recipeJson?.image || ''),
  };
}
...
function buildSearchQueries({ intent, mainIngredient, availableIngredients, childrenMode, user }) {
  const timeLabel = intent?.time === 'short' ? 'quick' : intent?.time === 'medium' ? '30 minute' : 'weeknight';
...
function fallbackDescription(sourceRecipe, childrenMode) {
  ...
  return sourceRecipe.description || 'A web-inspired recipe adapted for this app.';
}

function hasUnsafeRecipeTerms(recipe) {
  const text = `${recipe.title} ${recipe.description} ${(recipe.tags || []).join(' ')} ${(recipe.ingredients || []).map(ingredient => ingredient.name).join(' ')} ${(recipe.steps || []).map(step => step.instruction).join(' ')}`.toLowerCase();
...
function getProviderState() {
  ...
  if (PREFERRED_SEARCH_PROVIDER === 'tavily' && tavilyConfigured) activeSearchProvider = 'tavily';
  else if (PREFERRED_SEARCH_PROVIDER === 'serpapi' && serpApiConfigured) activeSearchProvider = 'serpapi';
...
function normalizeGeneratedRecipe(recipe, index, options = {}) {
  ...
  const description = normalizeText(recipe.description || 'AI-generated recipe inspired by internet sources.');
  const cuisine = normalizeText(recipe.cuisine || 'Fusion');
  const difficulty = ['easy', 'medium', 'hard'].includes(recipe.difficulty) ? recipe.difficulty : (childrenMode ? 'easy' : 'medium');
  const ingredients = (recipe.ingredients || []).map(normalizeIngredient).filter(Boolean).slice(0, 30);
  const steps = (recipe.steps || []).map((step, stepIndex) => normalizeStep(step, stepIndex)).filter(Boolean).slice(0, 12);
  const tags = Array.isArray(recipe.tags) ? recipe.tags.map(tag => normalizeText(tag)).filter(Boolean) : [];
  const recipeTags = childrenMode ? Array.from(new Set([...tags, 'kid-friendly'])) : tags;
  ...
  return {
    ...
    imageUrl: recipe.imageUrl || getGeneratedImageUrl(imagePrompt, { seed: index + 11 }),
    isGenerated: true,
  };
}
...
function fallbackFromSources(sourceRecipes, options) {
  ...
  return filtered.slice(0, 6).map((recipe, index) => normalizeGeneratedRecipe({
    ...recipe,
    title: options.childrenMode ? `${normalizeText(recipe.title)} Family Style` : recipe.title,
    description: fallbackDescription(recipe, options.childrenMode),
    difficulty: options.childrenMode ? 'easy' : recipe.difficulty,
    cookingTime: options.childrenMode ? Math.min(Number(recipe.cookingTime || 20), 30) : recipe.cookingTime,
    prepTime: options.childrenMode ? Math.min(Number(recipe.prepTime || 15), 15) : recipe.prepTime,
    steps: options.childrenMode
      ? (recipe.steps || []).map((step, stepIndex) => ({
          ...step,
          order: Number(step.order || stepIndex + 1),
          instruction: normalizeText(step.instruction || step.text || step.name || ''),
          tip: normalizeText(step.tip || 'Keep the steps simple and let children help with safe mixing and measuring tasks.'),
          whyItMatters: normalizeText(step.whyItMatters || 'This keeps the recipe easy to follow and child-friendly.'),
        }))
      : recipe.steps,
    tags: options.childrenMode ? [...(recipe.tags || []), 'kid-friendly', 'family'] : recipe.tags,
  }, index, options));
}

function buildDeterministicChildrenFallback(sourceRecipes, requestContext) {
  const primarySource = sourceRecipes[0] || {};
  const mainIngredient = normalizeText(requestContext.mainIngredient || requestContext.availableIngredients?.[0] || 'pasta');
  const pantryIngredients = Array.isArray(requestContext.availableIngredients)
    ? requestContext.availableIngredients.slice(0, 4).map(item => normalizeText(item)).filter(Boolean)
    : [];
  const ingredientNames = Array.from(new Set([mainIngredient, ...pantryIngredients, 'olive oil', 'mild cheese', 'herbs']))
    .filter(Boolean)
    .slice(0, 6);
    ...
    steps: [
      {
        ...
        whyItMatters: 'Organizing ingredients first keeps the cooking process calm and easy to follow.',
      },
      {
        order: 2,
        ...
        whyItMatters: 'Gentle cooking keeps the texture soft and the flavor approachable.',
      },
      {
        order: 3,
        ...
        whyItMatters: 'A simple finishing step helps children feel included without adding risk.',
      },
    ],
  ...
  return [normalizeGeneratedRecipe(recipe, 0, {
    childrenMode: true,
    source: primarySource.source || null,
    sourceImageUrl: primarySource.sourceImageUrl || '',
  })];
  ...
  const prompt = {
    ...
    outputSchema: {
      recipes: [{
        ...
        steps: [{ instruction: 'string', tip: 'string', whyItMatters: 'string' }],
        tags: ['string'],
...
async function suggestInternetRecipes({ user, ...requestContext }) {
  ...
  if (sourceRecipes.length === 0) {
    return {
      ...
      metadata: {
        providerState,
        warnings: [providerState.searchConfigured ? 'No recipe sources were extracted from search results.' : 'No search provider is configured, so internet recipe search is disabled.'],
  ...
  let mains = (generatedRecipes && generatedRecipes.length > 0
    ? generatedRecipes.map((recipe, index) => {
        const source = sourceRecipes[recipe.sourceIndex] || sourceRecipes[index] || null;
    ...
    : fallbackFromSources(sourceRecipes, { childrenMode: requestContext.childrenMode }))
    .filter(recipe => recipe.ingredients.length > 0 && recipe.steps.length > 0)
    .filter(recipe => !requestContext.childrenMode || ensureKidFriendlyRecipe(recipe))
    .slice(0, 6);
  ...
  if (mains.length === 0) {
    return {
      ...
      metadata: {
        providerState,
        warnings: [requestContext.childrenMode ? 'No child-friendly recipes passed validation.' : 'No valid generated recipes were produced.'],
        fallbackReason: requestContext.childrenMode ? 'children-validation-empty' : 'generation-empty',
      },
    };
  }

  const warnings = [];
  if (!providerState.geminiConfigured) warnings.push('Gemini is not configured, so results were adapted directly from extracted source recipes.');
  if (!providerState.searchConfigured) warnings.push('No search provider is configured. Only local recipes are available until search credentials are added.');
  if (requestContext.childrenMode) warnings.push('Children mode is enforcing stricter ingredient and language filtering.');
  ...
  return {
    ...
    metadata: {
      providerState,
      warnings,
      fallbackReason: generatedRecipes && generatedRecipes.length > 0 ? null : 'used-extracted-sources',
    },
  };
}

...
function dedupeRecipes(recipes) {
  ...
  return (recipes || []).filter(r => {
    const key = String(r?.title || r?.source?.url || r?._id || '').toLowerCase().trim();
  ...
  try {
    ...
    if (dietary) {
      dietary.split(',').forEach(d => {
        filter[`dietaryInfo.is${d.charAt(0).toUpperCase() + d.slice(1)}`] = true;
      });
    }
...
router.get('/search', async (req, res) => {
  try {
    ...
    if (spoonacular.isEnabled()) {
      const results = await spoonacular.searchRecipes(q.trim(), { number: 12, diet, cuisine, maxReadyTime: maxTime ? parseInt(maxTime) : undefined });
      if (results) return res.json(results);
    }
    const results = await searchMeals(q.trim());
    res.json(results);
  } catch (err) { res.status(500).json({ message: 'Search failed', error: err.message }); }
});
...
router.get('/spoonacular/:spoonacularId', async (req, res) => {
  try {
    ...
    if (!recipe) {
      ...
      recipe = await Recipe.findOneAndUpdate({ spoonacularId }, { $setOnInsert: data }, { upsert: true, new: true });
    }
    res.json(recipe);
  } catch (err) { res.status(500).json({ message: err.message }); }
});
...
router.get('/mealdb/:mealdbId', async (req, res) => {
  try {
    ...
    if (!recipe) {
      ...
      recipe = await Recipe.findOneAndUpdate({ mealdbId }, { $setOnInsert: data }, { upsert: true, new: true });
    }
    res.json(recipe);
  } catch (err) { res.status(500).json({ message: err.message }); }
});
...
router.post('/suggest', auth, async (req, res) => {
  try {
    ...
    const internetResult = await suggestInternetRecipes({ user, intent, availableIngredients, maxTime, difficulty, mainIngredient, childrenMode });
    ...
    if (internetResult?.mains?.length) {
      ...
      const mains = enrichRecipeResults(storedMains, { popularityMap, availableIngredients, mainIngredient, resultType: 'generated' });
      const side = storedSide ? enrichRecipeResults([storedSide], { popularityMap, availableIngredients, mainIngredient, resultType: 'generated' })[0] : null;
      return res.json({ mains, side, mode: 'generated', metadata: internetResult.metadata || null, sources: internetResult.sources || [] });
    }
    ...
    const mains = enrichRecipeResults(uniqueMains, { popularityMap, availableIngredients, mainIngredient, resultType: 'database' });
    const side = result.side ? enrichRecipeResults([result.side], { popularityMap, availableIngredients, mainIngredient, resultType: 'database' })[0] : null;
    const warnings = [];
    ...
    res.json({ mains, side, mode: 'database', metadata: { providerState: internetResult?.metadata?.providerState || null, warnings, fallbackReason: internetResult?.metadata?.fallbackReason || 'ai-unavailable' }, sources: internetResult?.sources || [] });
  } catch (err) { res.status(500).json({ message: err.message }); }
});
...
router.get('/:id/substitutions', async (req, res) => {
  try {
    const recipe = await Recipe.findById(req.params.id);
    if (!recipe) return res.status(404).json({ message: 'Recipe not found' });
    res.json({ substitutionGuide: recipe.substitutionGuide || [], ingredients: recipe.ingredients.map(ing => ({ name: ing.name, substitutions: ing.substitutions || [] })) });
  } catch (err) { res.status(500).json({ message: err.message }); }
});
...
router.get('/filter/quick-meals', async (req, res) => {
  try {
    res.json(await Recipe.find({ $expr: { $lte: [{ $add: ['$cookingTime', '$prepTime'] }, 30] } }));
  } catch (err) { res.status(500).json({ message: err.message }); }
});

router.get('/filter/by-energy/:energyLevel', async (req, res) => {
  try {
    res.json(await Recipe.find({ energyLevelRequired: req.params.energyLevel }));
  } catch (err) { res.status(500).json({ message: err.message }); }
});
...
router.post('/:id/increment-cook-count', auth, async (req, res) => {
  try {
    const recipe = await Recipe.findById(req.params.id);
    if (!recipe) return res.status(404).json({ message: 'Recipe not found' });
    if (!recipe.communityRatings) recipe.communityRatings = { averageRating: 0, totalRatings: 0, completionRate: 0 };
    recipe.communityRatings.completionRate = ((recipe.communityRatings.completionRate || 0) * (recipe.communityRatings.totalRatings || 1) + 1) / ((recipe.communityRatings.totalRatings || 0) + 1);
    await recipe.save();
    res.json(recipe);
  } catch (err) { res.status(500).json({ message: err.message }); }
});

...
const { ExperienceSession, UserExperiencePreferences } = require('../models/Experience');

router.post('/session/start', auth, async (req, res) => {
  try {
    const { recipeId, musicSettings, ambianceSettings, socialCooking } = req.body;
...
router.get('/preferences', auth, async (req, res) => {
  try {
    let preferences = await UserExperiencePreferences.findOne({ userId: req.userId });
...
router.put('/preferences', auth, async (req, res) => {
  try {
    ...
    if (!preferences) {
      preferences = new UserExperiencePreferences({ userId: req.userId });
    }
...
router.get('/sessions/recent', auth, async (req, res) => {
  try {
    const sessions = await ExperienceSession.find({ userId: req.userId })

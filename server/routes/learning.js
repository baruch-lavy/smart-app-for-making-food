...
router.get('/techniques', auth, async (req, res) => {
  try {
    const techniques = await Technique.find({}).sort({ category: 1, difficulty: 1 });
    res.json(techniques);
  } catch (error) {
...
router.get('/techniques/:id', auth, async (req, res) => {
  try {
    const technique = await Technique.findById(req.params.id).populate('relatedRecipes');
    res.json(technique);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
router.get('/chef-notes/recipe/:recipeId', auth, async (req, res) => {
  try {
    const notes = await ChefNote.find({ recipeId: req.params.recipeId }).sort({ stepNumber: 1 });
    res.json(notes);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
router.get('/path', auth, async (req, res) => {
  try {
    let learningPath = await LearningPath.findOne({ userId: req.userId });
...
router.post('/path/complete-technique', auth, async (req, res) => {
  try {
    ...
    if (!learningPath) {
      learningPath = new LearningPath({ userId: req.userId });
    }
